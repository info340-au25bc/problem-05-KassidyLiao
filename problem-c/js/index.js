...
let huskyGames2016 = JSON.parse(huskyGames2016Json);
...
let fumbleGames = huskyGames2016.filter(function (game) {
  return game["fumbles"] && game["fumbles"] > 0;
});
...
let totalDiff = huskyGames2016.reduce(function (acc, game) {
  let uwScore =
    game["home"] === "UW" ? game["home_score"] : game["opponent_score"];
  let oppScore =
    game["home"] === "UW" ? game["opponent_score"] : game["home_score"];
  return acc + (uwScore - oppScore);
}, 0);
let avgScoreDifference = totalDiff / huskyGames2016.length;
